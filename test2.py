...
class MainFrame():
    ...
    def weather(self, now):
        ...
        # 時間ごとのデーター処理
        # 現在時刻から6時間ごとの降水確率をリストへ入れる
        # 43 - (6 * (list_index)) で翌日の18時以降のデータをリストに入れない
        for i in range(0, (43 - (6 * (list_index))), 6):
            hour_weather_date = json_date["hourly"][i]
            ...
            # リストに入れるインデックスの計算脳の変数
            count = int(i / 6)
            # リストへ格納
            rainy_percent_list[list_index + count] = (str_rainy)
